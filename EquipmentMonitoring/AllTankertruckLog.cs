...
{
    public partial class AllTankertruckLog : Form
    {
        ...
        private void CategoryParent(string Fuel)
        {
            using (SqlConnection Con = new SqlConnection(Connection))
            {
                SqlDataAdapter da = new SqlDataAdapter("Select * from m_TankList with(nolock) where IsMainTank = 0 and TypeOfFuel = '" + Fuel + "'", Con);
                DataTable dt = new DataTable();
                da.Fill(dt);
